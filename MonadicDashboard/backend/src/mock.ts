...
export interface NugetPackageMock {
  id: string;
  version: string;
...
export interface ActivityEventMock {
  id: string;
  repo: string;
  type: 'PushEvent' | 'ReleaseEvent' | 'PullRequestEvent' | 'CreateEvent';
  actor: string;
  description: string;
  createdAt: string;
}

export const MOCK_REPOS: RepoMock[] = [
  {
    name: 'MonadicSharp',
    stars: 42,
    forks: 8,
...
export const MOCK_NUGET_PACKAGES: NugetPackageMock[] = [
...
function randomSeries(base: number, days = 7): number[] {
  const result: number[] = [];
  let val = base;
...
export const MOCK_ACTIVITIES: ActivityEventMock[] = [
...
export function mockNugetStats() {
  const totalDownloads = MOCK_NUGET_PACKAGES.reduce((sum, p) => sum + p.totalDownloads, 0);
  ...
  return {
    totalDownloads,
    packages: MOCK_NUGET_PACKAGES,
    weekSeries,
    deltaPercent: parseFloat(delta.toFixed(1)),
  };
}

export function mockGithubStars() {
...
export function mockGithubActions() {
  const runs = MOCK_REPOS.map((r) => ({
    repo: r.name,
    status: r.ciStatus,
...
export function mockEcosystemHealth() {
  return {
...
export function mockGreenScore() {
  const scores = MOCK_REPOS.map((r) => {
    let score = 60;
    if (r.ciStatus === 'passing') score += 20;
    if (r.nugetDownloads > 500) score += 10;
    if (r.stars > 20) score += 10;
